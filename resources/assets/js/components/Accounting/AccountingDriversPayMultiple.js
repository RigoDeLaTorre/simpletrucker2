...
class AccountingDriversPayMultiple extends Component {
  state = {
    focused: "",
    date: ""
  };
              ...
              <li key={index} className="load-item">
                ...
                <div className="top-details">
                  <h4 className="top-details__listItem">
                    Load Id :{this.props.currentLoadsSelected[index].invoice_id}
                  </h4>
                  <h4 className="top-details__listItem">
                    Customer:{" "}
                    {
                  ...
                  <h4 className="top-details__listItem">
                    Pickups{" "}
                    {this.props.currentLoadsSelected[index].pickups.length}
                  </h4>
                  <h4 className="top-details__listItem">
                    Deliveries{" "}
                    {this.props.currentLoadsSelected[index].deliveries.length}
                  </h4>
                </div>

                <h3 className="load-details__startingPoint">
                  Starting Point :This will be a new field you add to the "Add
                  new load", possibly add ending point as well.{" "}
                </h3>
                ...
                <div className="payContainer">
                  <div className="load-form-section">
                    <div className="row load-form-section__topGroup">
                      <div className="load-form-section__topGroup__group">
                        <Field
                          label="Pay Type"
                          name={`loadSelected[${index}].driver_paid_type`}
                          className="formSelectFields col s3"
                          type="select"
  ...
  render() {
    ...
    return (
      <div id="driverPayMultipleModal" className="driverPayModal">
        <div className="app-container">
          <div className="top-section">
            <h1 className="top-section__title">Accounting -Drivers Payroll</h1>

            <div className="top-section__driverInfo">
              <h4 className="top-section__driverInfo__detail top-section__driverInfo__driverName">
                Driver :{" "}
                {this.props.currentLoadsSelected[0].driver.driver_first_name}{" "}
                {this.props.currentLoadsSelected[0].driver.driver_last_name}
              </h4>
              <h4 className="top-section__driverInfo__detail">
                Phone:{" "}
                {normalizePhone(
              ...
              <h4 className="top-section__driverInfo__detail">
                Email: {this.props.currentLoadsSelected[0].driver.driver_email}
              </h4>
              <h4>
                Salary:{" "}
                {this.props.currentLoadsSelected[0].driver.driver_pay_amount}{" "}
              </h4>
            </div>
          </div>
...
AccountingDriversPayMultiple = reduxForm({
  form: "DriverPayrollMultiple"
})(AccountingDriversPayMultiple);

AccountingDriversPayMultiple = connect(
  state => ({
    currentLoadsSelected: state.accounting.loadSelected,
    currentForm: state.form.DriverPayrollMultiple,
...
export default AccountingDriversPayMultiple;
