...
import { updateCompany } from '../../actions/company/updateCompany.js'
import {
  required,
  maxLength,
  maxLength10,
...
class ProfileEdit extends Component {
  ...
  onSubmit(values) {
    ...
    if (
      ...
      M.toast({
        html: `${newValues.company_bill_name.toUpperCase()} has been updated !`,
  ...
  render() {
    ...
    return (
      <section id="profile-edit" className="app-container">
...
ProfileEdit = connect(
  state => ({
    ...
    synchronousError: getFormSyncErrors('profileEdit')(state),
    ...
    initialValues: {
      ...state.company,
      company_state: stateOptions.find(
        s => s.value === state.company.company_state
      ),
      company_bill_state: stateOptions.find(
        s => s.value === state.company.company_bill_state
      ),
      factory_company_state: stateOptions.find(
        s => s.value === state.company.factory_company_state
      ),
  ...
  { updateCompany: updateCompany } // bind account loading action creator
)(ProfileEdit)
